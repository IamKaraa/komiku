...
<div class="flex h-screen">
    ...
    <aside id="sidebar" class="w-64 -translate-x-full lg:translate-x-0 fixed lg:relative z-20 flex-shrink-0 rounded-tl-3xl flex flex-col text-[#E0E1DD] bg-[#1B263B] transition-transform duration-300 ease-in-out">
        ...
        <nav class="flex-grow px-4">
            ...
            <a href="{{ route('admin.users.index') }}" class="nav-item flex items-center py-3 px-4 rounded-lg transition-all duration-300 mt-2 transform hover:scale-105 {{ Request::is('admin/manage-user*') ? 'active' : '' }}">
                <i data-lucide="users" class="w-5 h-5 mr-3"></i>
                <span>Manage User</span>
            </a>
            <a href="{{ route('admin.comics.index') }}" class="nav-item flex items-center py-3 px-4 rounded-lg transition-all duration-300 mt-2 transform hover:scale-105 {{ Request::is('admin/manage-comic*') ? 'active' : '' }}">
                <i data-lucide="book-marked" class="w-5 h-5 mr-3"></i>
                <span>Validation Comic</span>
            </a>
            <a href="{{ route('admin.genres.index') }}" class="nav-item flex items-center py-3 px-4 rounded-lg transition-all duration-300 mt-2 transform hover:scale-105 {{ Request::is('admin/manage-genre*') ? 'active' : '' }}">
        ...
        <header class="h-24 flex items-center justify-between px-6 lg:px-10 rounded-tr-3xl bg-[#0D1B2A]">
            ...
            <div class="flex items-center space-x-6">
                ...
                <form action="{{ route('logout') }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="flex items-center space-x-2 text-gray-400 hover:text-red-400 transition-colors duration-300">
