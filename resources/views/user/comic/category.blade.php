...
@section('title', 'KOMIKU - ' . ($selectedGenre ? $selectedGenre->name : 'All Categories'))
    ...
    <section class="section mb-8 mx-4">
        <div class="text-center">
            <h1 class="text-3xl lg:text-4xl font-bold text-gray-900 mb-4">
                {{ $selectedGenre ? $selectedGenre->name . ' Comics' : 'All Comics' }}
            </h1>
            <p class="text-lg text-gray-600">
                {{ $selectedGenre ? 'Discover amazing ' . strtolower($selectedGenre->name) . ' stories' : 'Explore all available comics' }}
            </p>
        </div>
    </section>
    ...
    <section class="section mb-8 mx-4">
        <div class="flex flex-col lg:flex-row justify-between items-center gap-4 w-full max-w-7xl mx-auto">
            ...
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('comics.category') }}"
                   class="chip {{ !$selectedGenre ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 hover:bg-gray-200' }} px-4 py-2 rounded-full text-sm font-medium transition duration-300">
                    All
                </a>
                @foreach($genres as $genre)
                    <a href="{{ route('comics.category', $genre->slug) }}"
                       class="chip {{ $selectedGenre && $selectedGenre->id == $genre->id ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 hover:bg-gray-200' }} px-4 py-2 rounded-full text-sm font-medium transition duration-300">
                        {{ $genre->name }}
                    </a>
                @endforeach
            </div>
        ...
        @else
            <div class="text-center py-16">
                ...
                <p class="text-gray-600">
                    {{ $selectedGenre ? 'No comics available in this category yet.' : 'No comics available at the moment.' }}
                </p>
                @if($selectedGenre)
                    <a href="{{ route('comics.category') }}" class="inline-block mt-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-6 rounded-lg transition duration-300">
                        View All Comics
                    </a>
                @endif
            </div>
